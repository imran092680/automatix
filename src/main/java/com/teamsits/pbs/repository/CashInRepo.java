...
public interface CashInRepo extends JpaRepository<CashIn, Long> {
    @Query("SELECT ci FROM CashIn ci WHERE ci.transactionDate = :date AND ci.isDeleted = com.teamsits.pbs.utils.ApplicationConstant.DOMAIN_STATUS_ZERO ORDER BY ci.transactionDate ASC, ci.id ASC")
    List<CashIn> findCashInsByDateWhereIsDeletedEqualsZero(@Param("date") LocalDate date);

    @Query("SELECT ci FROM CashIn ci WHERE ci.id = :cId AND ci.isDeleted = com.teamsits.pbs.utils.ApplicationConstant.DOMAIN_STATUS_ZERO")
